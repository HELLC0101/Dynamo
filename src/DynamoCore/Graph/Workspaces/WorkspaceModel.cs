            ...
            // using (UndoRecorder.BeginActionGroup())
            {
                foreach (var nodeList in cliques)
                {
                    ...
                    foreach (var node in nodeList)
                    {
                        ...
                        {
                            if (!NodeToCode.IsInternalNodeToCodeConnection(nodeList, connector))
                            {
                    ...
                    var newInputConnectors = NodeToCode.ReConnectInputConnections(externalInputConnections, codeBlockNode, this);
                    foreach (var connector in newInputConnectors)
                    {
                        undoHelper.RecordCreation(connector);
                    }

                    var newOutputConnectors = NodeToCode.ReConnectOutputConnections(externalOutputConnections, codeBlockNode);
